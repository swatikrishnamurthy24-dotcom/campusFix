...
import AssignStaffDialog from "@/components/issues/AssignStaffDialog"
  ...
  if (loadError || !issue) {
    return (
      <div className="mx-auto flex max-w-md flex-col items-center gap-2 py-16 text-center">
        ...
        <Link to="/admin/issues" className="mt-2 text-sm font-medium text-primary hover:underline">
          Back to All Issues
        </Link>
      </div>
    )
    ...
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      <Card>
        <CardContent className="p-6">
          ...
          {!issue.image && (
            <p className="mt-4 text-xs text-muted-foreground">No attachments for this issue.</p>
          )}
        ...
        <CardContent className="flex flex-wrap items-center justify-between gap-3 p-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {issue.assignedToName ? issue.assignedToName : "Unassigned"}
            </p>
          </div>
          <AssignStaffDialog issue={issue} onAssigned={setIssue} />
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="p-6">
          <p className="mb-3 text-sm font-semibold text-foreground">Add an Admin Comment</p>
          <form onSubmit={handleCommentSubmit} className="space-y-2">
            <Textarea
              placeholder="Add a note, decision, or escalation for this issue…"
              value={commentDraft}
              onChange={(e) => {
                setCommentDraft(e.target.value)
                if (commentError) setCommentError("")
              }}
              aria-label="Add an admin comment"
