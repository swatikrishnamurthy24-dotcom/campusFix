...
export default function AssignedIssues() {
  ...
  useEffect(() => {
    ...
    getAssignedIssues(user.id)
      ...
      .catch(() => {
        if (!cancelled) setLoadError("Couldn't load your assigned issues right now. Please try again.")
      })
      .finally(() => {
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Issues currently assigned to you.</p>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-surface p-3 sm:flex-row sm:flex-wrap sm:items-center">
        <div className="relative flex-1 sm:min-w-[200px]">
          ...
          <Input
            placeholder="Search by title, ID, location, or student"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="pl-8"
            aria-label="Search assigned issues"
          />
        </div>
