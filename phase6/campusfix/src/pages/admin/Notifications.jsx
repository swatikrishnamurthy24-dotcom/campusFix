...
import { Bell, CheckCheck, Siren, AlertTriangle, UserPlus, CheckCircle2, Clock, FilePlus2, BellOff } from "lucide-react"
import { useAuth } from "@/hooks/useAuth"
import { getNotifications, markNotificationRead, markAllNotificationsRead } from "@/services/issueService"
import { Button } from "@/components/ui/button"
import { cn } from "@/lib/utils"

// Admin notification types (Phase 6 §12) — separate icon map, same
// getNotifications()/markNotificationRead() abstraction as the Student and
// Staff portals (filtered by userId).
const TYPE_ICON = {
  emergency: Siren,
        ...
        <ul className="space-y-2">
          {notifications.map((n) => {
            ...
            return (
              <li key={n.id}>
                {n.issueId ? (
                  <Link to={`/admin/issues/${n.issueId}`} onClick={() => !n.read && handleMarkRead(n.id)}>
                    {content}
                  </Link>
                ) : (
